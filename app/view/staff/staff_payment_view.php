...
<div class="content1"  style="background-color: white;border:none">
    <div   id="dis" style="width: 100%; margin:1px; padding: 2px;height: 100%;overflow:auto;">
            <h4 style="float: none; padding-left: 10px; color:#333">Transactions</h4><br/>

<?php

$result=$_SESSION['payment'];
if($result!='false'){
foreach($result as $key => $value) {
  


  $pReceieve=$result[$key]['RECIEVE'];
  $bonus=$result[$key]['BONUS'];
  $jobId=$result[$key]['JOBID'];
  $pBy=$result[$key]['PAYEDBY'];
  $date=$result[$key]['DATE'];

echo "<div class='divFlot' style='width:900px;height:130px;margin:0 auto;background-color:#f1f1f1 '>
  <div  style='width:895px;height:30px;margin:0 auto;margin-top: 5px;text-align: left;padding-left:5px '>Payment Id : $key</div>
  <div style='width:895px;height:50px;margin:0 auto;margin-top: 5px;text-align: left;padding-left:5px'>Details<br><span>Payed For Job:$jobId</span><span style='margin-left:20px'> Payment Receieve:$pReceieve</span><span style='margin-left:20px'>Bonus:$bonus</span><br>
  <span>Payed By:$pBy</span><span style='margin-left:20px'> Payment Date:$date</span></div>
</div><br>";
