...
<div class="content1"  style="background-color: white;border:none">
    <div   id="dis" style="width: 100%; margin:1px; padding: 2px;height: 100%;overflow:auto;">
            <h4 style="float: none; padding-left: 10px; color:#333">Applied Job List</h4><br/>

<?php

$result=$_SESSION['jobApplied'];
if($result!='false'){
foreach ($result as $key => $value) {
  


  $jobTitle=$result[$key]['TITLE'];
  $jobDepartment=$result[$key]['DEPARTMENT'];
  $jobType=$result[$key]['TYPE'];
  $jobDate=$result[$key]['DATE'];
  $jobConfirm=$result[$key]['CONFIRMATION'];
  $jobConfirmBy=$result[$key]['CONFIRMATIONBY'];

  if($jobConfirm=="Confirmed")
  {
    $vlu="green";
  }
  else
...
echo "<div class='divFlot' style='width:900px;height:130px;margin:0 auto;background-color:#f1f1f1 '>
  <div  style='width:895px;height:30px;margin:0 auto;margin-top: 5px;text-align: left;padding-left:5px '>JOBID : $key</div>
  <div style='width:895px;height:50px;margin:0 auto;margin-top: 5px;text-align: left;padding-left:5px'>Details<span style='float:right;margin-right:10px;color:$vlu'>$jobConfirm By $jobConfirmBy</span><br><span>Title:$jobTitle</span><span style='margin-left:20px'> Department:$jobDepartment</span><br><span> Applied Date:$jobDate</span></div>
  <div class='divFlot' style='width:895px;height:30px;margin:0 auto;margin-top: 5px;text-align: left;padding-right:5px;text-align:right;padding:5px '><a   class='aButton' style='margin-left:10px' onclick='getJobDetailsHome($key,1)' >More Details..</a></div>
</div><br>";
