...
{
  if (isset($_SESSION['showTransactionDetails'])) {
     $showTransactionDetails=$_SESSION['showTransactionDetails'];
...
<div id='dis'>
        <form method="POST">
            <input type="submit" name="paySalary" value="Pay Salary To Staff" formaction="?chief_paySalary" style="padding: 10px 18px;">
        </form>
        

            <br>
            <h4 style="padding-left: 10px; color:#333"><br/>Transaction History</h4><br/>
...
<table style="width: 100%; margin:1px; padding: 2px; border: 1px solid black;">
            ...
            <tr>
                ...
                <td style="width: 8%">Tranaction ID</td>      
                <td style="width: 8%">Paid Amount</td>   
                <td style="width: 8%">Extra Paid Amount</td> 
                <td style="width: 7%">Job ID</td> 
                <td style="width: 8%">Paid Date</td>    
               
            </tr>
            <?php
            if (isset($showTransactionDetails)) {
            foreach ($showTransactionDetails as $row) {
              echo  '<tr>
                <td style="width: 8%">'.$row['ACCID'].'</td> 
                <td style="width: 6%">'.$row['NAME'].'</td>
                <td style="width: 8%">'.$row['TRANSACTIONID'].'</td> 
                <td style="width: 8%">'.$row['U_PAY'].'</td>     
                <td style="width: 8%">'.$row['EXTRA_BONUS'].'</td>    
