...
{
  if (isset($_SESSION['thisNoticeDetails'])) {
     $thisNoticeDetails=$_SESSION['thisNoticeDetails'];
...
<div   class="content">
    <div style="width: 100%; margin:1px; padding-top: 20px; padding-bottom:10px;  border: 1px solid black; height: auto; ">
        ...
        <form method="POST">
            <fieldset>
            <legend >Sent Notice</legend>
            <br>
             <table> 
             <tr>
                <td align="left">Notice Title:
                     <div style="margin-top: -19px; text-align: center;" >  

                      <?php

                      if (isset($_POST['noticeTitle'])) {
                           echo '<input style="text-align: center;" type="text" placeholder="write  t i t l e" value="'.$_POST['noticeTitle'].'"  name="noticeTitle">';
                      }else
                       {
                         echo '<input style="text-align: center;" type="text" placeholder="write  t i t l e" value=""  name="noticeTitle">';
                        }
                     ?>
                      </div>
                </td>
                <td align="left">Notice :
                     <div style="margin-top: -19px; text-align: center;" > 
                      <?php
                      if (isset($_POST['noticeDetails'])) {
                            echo '<textarea name="noticeDetails" rows="4" cols="20" placeholder="Write Your Notice">'.$_POST['noticeDetails'].'</textarea>';
                      }else
                       {
                         echo '<textarea name="noticeDetails" rows="4" cols="20" placeholder="Write Your Notice"></textarea>';
                        }
                     ?>           
                    </div>
                </td>
           ...
           <?php  echo '<td colspan="2"><button name="sentThisNotice" value="sentThisNotice" formaction="?chief_sendThisNotice&teamId='.$_GET['teamId'].'" style="padding: 10px 18px;">Sent Notice</button></td>' ; ?>
...
<?php
        ...
        if (isset($_POST['sentThisNotice'])) {

        $noticeTitle=trim($_POST['noticeTitle']," ");
        $noticeDetails=trim($_POST['noticeDetails']," ");
        
        if ($noticeDetails==""){  
         echo '<span id="validationError">Notice Title Can not be empty</span>';
        }else if ($noticeDetails=="") {
            echo '<span id="validationError">Notice Can not be empty</span>';
        }else{
